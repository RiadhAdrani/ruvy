...
`,e=e.substring(m.length+1),B=!0),!B){const v=new RegExp(`^ {0,${Math.min(3,i-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),y=new RegExp(`^ {0,${Math.min(3,i-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),b=new RegExp(`^ {0,${Math.min(3,i-1)}}(?:\`\`\`|~~~)`),E=new RegExp(`^ {0,${Math.min(3,i-1)}}#`);for(;e&&(f=e.split(`
`,1)[0],m=f,this.options.pedantic&&(m=m.replace(/^ {1,4}(?=( {4})*[^ ])/g,"  ")),!(b.test(m)||E.test(m)||v.test(m)||y.test(e)));){if(m.search(/[^ ]/)>=i||!m.trim())g+=`
`+m.slice(i);else{if(l||p.search(/[^ ]/)>=4||b.test(p)||E.test(p)||y.test(p))break;g+=`
`+m}!l&&!m.trim()&&(l=!0),o+=f+`
`,e=e.substring(f.length+1),p=m.slice(i)}}F.loose||(u?F.loose=!0:/\n *\n *$/.test(o)&&(u=!0)),this.options.gfm&&(r=/^\[[ xX]\] /.exec(g),r&&(a=r[0]!=="[ ] ",g=g.replace(/^\[[ xX]\] +/,""))),F.items.push({type:"list_item",raw:o,task:!!r,checked:a,loose:!1,text:g}),F.raw+=o}F.items[F.items.length-1].raw=o.trimRight(),F.items[F.items.length-1].text=g.trimRight(),F.raw=F.raw.trimRight();const d=F.items.length;for(s=0;s<d;s++)if(this.lexer.state.top=!1,F.items[s].tokens=this.lexer.blockTokens(F.items[s].text,[]),!F.loose){const v=F.items[s].tokens.filter(b=>b.type==="space"),y=v.length>0&&v.some(b=>/\n.*\n/.test(b.raw));F.loose=y}if(F.loose)for(s=0;s<d;s++)F.items[s].loose=!0;return F}}html(e){const t=this.rules.block.html.exec(e);if(t){const o={type:"html",block:!0,raw:t[0],pre:!this.options.sanitizer&&(t[1]==="pre"||t[1]==="script"||t[1]==="style"),text:t[0]};if(this.options.sanitize){const r=this.options.sanitizer?this.options.sanitizer(t[0]):V(t[0]);o.type="paragraph",o.text=r,o.tokens=this.lexer.inline(r)}return o}}def(e){const t=this.rules.block.def.exec(e);if(t){const o=t[1].toLowerCase().replace(/\s+/g," "),r=t[2]?t[2].replace(/^<(.*)>$/,"$1").replace(this.rules.inline._escapes,"$1"):"",a=t[3]?t[3].substring(1,t[3].length-1).replace(this.rules.inline._escapes,"$1"):t[3];return{type:"def",tag:o,raw:t[0],href:r,title:a}}}table(e){const t=this.rules.block.table.exec(e);if(t){const o={type:"table",header:vo(t[1]).map(r=>({text:r})),align:t[2].replace(/^ *|\| *$/g,"").split(/ *\| */),rows:t[3]&&t[3].trim()?t[3].replace(/\n[ \t]*$/,"").split(`
...
`,xi=[{from:"0.5.0",md:wi}],Ne=n=>{const{children:e,class:t,isActive:o}=n,r=ge(()=>{let a="p-x-2 text-[0.9em]";return o?a+=" color-[var(--link)]":a+=" color-[var(--text-low)] hover:color-[var(--text-strong)]",Ko(a,t)},[t,o]);return createJsxElement("a",{...n,class:r},e)},vr=()=>createJsxElement("div",null,createJsxElement("h1",null,"Not found"),createJsxElement(Ne,{href:"/",class:"p-x-0!"},"Go back to Home page")),Tt=({versions:n})=>{const{version:e}=$t(),t=ge(()=>{const o=fe.indexOf(e);if(o===-1)throw"invalid version";for(const r of n){if(r.from===e||r.to===e)return r.md;const a=fe.indexOf(r.from);if(r.to){const i=fe.indexOf(r.to);if(a<=o&&o<=i)return r.md}else if(a<=o)return r.md}},[e,n]);return t?createJsxElement(dt,{content:t}):createJsxElement(vr,null)},ki=()=>createJsxElement(createJsxFragmentElement,null,createJsxElement(Tt,{versions:xi})),Di="data:text/markdown;base64,",Ei=()=>{const n=br(Di);return createJsxElement(createJsxFragmentElement,null,createJsxElement(dt,{content:n}))},Fi="data:text/markdown;base64,IyBNYWRlIHdpdGggUnV2eQoKIyMjIGBVbmRlciBjb25zdHJ1Y3Rpb24uLi5gCg==",Ci=()=>{const n=br(Fi);return createJsxElement(createJsxFragmentElement,null,createJsxElement(dt,{content:n}))},Ai=`# Types
...
`,Dl=[{from:"0.5.0",to:"0.5.0",md:xl},{from:"0.5.2",md:kl}],El=`# Portal
...
`,Cl=[{from:"0.5.0",to:"0.5.0",md:El},{from:"0.5.2",md:Fl}],Al=`# ErrorBoundary
...
\`\`\`
`,Tl=[{from:"0.5.7",md:Al}],Sl=`# useErrorBoundary
...
`,Pl=[{from:"0.5.0",to:"0.5.0",md:Rl},{from:"0.5.2",to:"0.5.6",md:Bl},{from:"0.5.7",md:Il}],Ml=`# Beyond React: Exploring Additional Features and Differences
...
`,Ll=[{from:"0.5.0",md:Ml}],Nl=`# Class attribute
...
`,Jl=[{from:"0.5.0",md:Hl}],jl=`# \`switch\` directive
...
\`\`\`
`,Wl=[{from:"0.5.0",md:jl}],Gl=`# Event modifiers
...
\`\`\`
`,Zl=[{from:"0.5.0",to:"0.5.0",md:Gl},{from:"0.5.2",md:ql}],Yl=[{path:"/mountApp",versions:Ui,title:"mountApp"},{path:"/createRouter",versions:Ri,title:"createRouter"},{path:"/createStore",versions:[{from:"0.5.0",to:"0.5.0",md:Ti}],title:"createStore"},{path:"/createComposable",versions:Ii,title:"createComposable"},{path:"/useState",versions:Hi,title:"useState"},{path:"/useEffect",versions:Wi,title:"useEffect"},{path:"/useMemo",versions:Zi,title:"useMemo"},{path:"/useCallback",versions:Vi,title:"useCallback"},{path:"/useContext",versions:ol,title:"useContext"},{path:"/useComposable",versions:al,title:"useComposable"},{path:"/useId",versions:el,title:"useId"},{path:"/useErrorBoundary",versions:_l,title:"useErrorBoundary"},{path:"/createContext",versions:Li,title:"createContext"},{path:"/getParams",versions:ll,title:"getParams"},{path:"/getSearchParams",versions:dl,title:"getSearchParams"},{path:"/getPathname",versions:ml,title:"getPathname"},{path:"/replace",versions:[{from:"0.5.0",to:"0.5.0",md:Do}],element:Do,title:"replace"},{path:"/navigate",versions:bl,title:"navigate"},{path:"/batch",versions:[{from:"0.5.0",to:"0.5.0",md:Eo}],element:Eo,title:"batch"},{path:"/outlet",versions:wl,title:"<Outlet/>"},{path:"/fragment",versions:Dl,title:"<Fragment/>"},{path:"/portal",versions:Cl,title:"<Portal/>"},{path:"/error-boundary",versions:Tl,title:"<ErrorBoundary/>"}],Xl=[{path:"/class-attribute",versions:Ul,title:"Class attribute"},{path:"/joinClasses",versions:zl,title:"joinClasses"},{path:"/if-directive",versions:Jl,title:"if directives"},{path:"/switch-directive",versions:Wl,title:"switch directives"},{path:"/event-modifiers",versions:Zl,title:"Event modifiers"}],Fo=[{path:"/api",title:"API",children:Yl,versions:Pl},{path:"/more",title:"More",children:Xl,versions:Ll},{path:"/types",title:"Types",children:[],versions:[{from:"0.5.0",md:Ai}]}],Vl=`# Tutorial : Todo
...
`,eu=[{from:"0.5.0",md:Kl}],tu=`# Quick Start
...
<br/>
`,ou=[{from:"0.5.0",to:"0.5.0",md:tu},{from:"0.5.2",md:nu}],ru=`# Setup a new Project
...
- SPA Router.
- key-value Store.
`,uu=()=>createJsxElement(dt,{content:lu}),cu=[{path:"/",element:createJsxElement(createJsxFragmentElement,null,createJsxElement(su,null),createJsxElement("div",{class:"w-100% overflow-x-hidden row-center p-x-6 m-t-[var(--nav-bar-height)]",style:{minHeight:"calc(100vh - var(--nav-bar-height))"}},createJsxElement("div",{class:"col max-w-1200px flex-1 self-stretch overflow-x-hidden p-y-12 md:p-y-10"},createJsxElement(st,null)))),children:[{path:"/*",element:createJsxElement(vr,null)},{path:"",title:"Home",element:createJsxElement(vi,null),name:"Home"},{path:"/changelogs",title:"Changelogs",element:createJsxElement(uu,null),name:"ChangeLogs"},{path:"/acknowledgment",title:"Acknowledgment",element:createJsxElement(Ei,null)},{path:"/examples",title:"Examples",element:createJsxElement(Ci,null)},{path:"/learn",title:"Learn",element:createJsxElement(To,{rootURL:"/learn",sideBarItems:Co}),children:Co.map(n=>({title:n.title,path:n.path,element:createJsxElement(Tt,{versions:n.versions})}))},{path:"/docs",title:"Docs",element:createJsxElement(To,{rootURL:"/docs",sideBarItems:Fo}),children:[{path:"",element:createJsxElement(ki,null)},...Fo.map(n=>({path:n.path,title:n.title,element:createJsxElement(st,null),children:[{path:"",element:createJsxElement(Tt,{versions:n.versions})},...n.children.map(e=>({path:e.path,title:e.title,element:createJsxElement(Tt,{versions:e.versions})}))]}))]}]}];$a({routes:cu,base:"/ruvy",transformTitle:n=>`${n} - Ruvy`,correctScrolling:!0});Ca({app:createJsxElement(ws,null),host:document.querySelector("#app")});
